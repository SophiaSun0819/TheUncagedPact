    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag ( Varyings i, float facing : VFACE) : SV_Target
            {
                ...
                // return finalEffect;
                float intensity = finalEffect.r * 0.7 + finalEffect.g * 0.2 + finalEffect.b * 0.1;
    ...
    SubShader
    {
        Pass
        {
            ...
            fixed4 frag (v2f i, fixed facing : VFACE) : SV_Target
            {
                ...
                // return finalEffect;
                // finalEffect 是颜色，我们改用亮度作为透明度
                float intensity = finalEffect.r * 0.7 + finalEffect.g * 0.2 + finalEffect.b * 0.1;

                // 你可以调整亮度曲线，让发光越亮透明度越高
                float alpha = saturate(intensity);

                // Passthrough shader 必须只输出 alpha
                return float4(0, 0, 0, alpha);
